...
| `conveyor_segment_load`           | Gauge   | `zone_id`    | Bags currently in transit per segment |
| `conveyor_transit_queue`          | Gauge   | —            | Bags in cross-terminal transit queue  |
| `conveyor_delay_total`            | Counter | `terminal`   | Bags that missed make-up deadline     |
...
| `envelope_invalid_total`          | Counter | `reason`     | Rejected Kafka envelopes              |
