...
- [x] **P1-1-1** — Integrate the adsb.lol community API (`/v2/lat/{lat}/lon/{lon}/dist/{dist}`) as the primary live ADS-B source, with OpenSky Network REST API (`/api/states/all`) as fallback. Poll every 30 real seconds and store the latest state vectors (lat/lon/altitude/heading/callsign) in an in-memory cache (Redis not needed — scope is small).
...
- [x] **P1-1-5** — Use ADS-B historical tracks (from OpenSky Network Zenodo monthly dumps) to calibrate the great-circle interpolation model: measure actual track deviation from great-circle and add a correction factor per route region (North Atlantic tracks deviate significantly due to NAT track system). _(Plan written in docs/lessons-learned/phase-1-adsb-calibration-plan.md; implementation deferred to data science sprint. Note: live ADS-B now uses adsb.lol as primary source.)_
