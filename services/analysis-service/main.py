"""analysis-service — Prescriptive digital twin engine.

Consumes events from all domain services to detect bottlenecks, generate
recommendations, run what-if analyses, and optionally auto-apply interventions.
...
async def lifespan(app: FastAPI):
    """Startup: Neo4j → Kafka → producer → WS callback → consumer task."""
    # 1. Wait for Neo4j
    ...
    # 4. Register WebSocket broadcast callback
    set_ws_broadcast(ws_broadcast)

    # 5. Start Kafka consumer as background task
    asyncio.create_task(run_consumer())

    logger.info("analysis-service startup complete")
    yield
